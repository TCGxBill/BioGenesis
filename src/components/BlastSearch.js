    ...
    <div class="panel active">
      ...
      <div class="panel-body" id="blast-result">
        <div class="empty-state">
          ...
          <p style="font-size:11px;color:var(--text-muted);margin-top:8px;">Results returned live fromncbi.nlm.nih.gov API (approx. 30-120 seconds)</p>
        </div>
      </div>
    </div>
  `;
...
export async function runBlast(query, program = 'blastn', database = 'nt') {
  ...
  try {
    ...
    let attempts = 0;
    const maxAttempts = 40; // Allow up to ~3 minutes
    ...
    while (attempts < maxAttempts) {
      ...
      if (checkText.includes('Status=READY')) {
        if (checkText.includes('ThereAreHits=yes')) {
          ...
          return '<div class="empty-state"><span class="empty-state-icon"></span><p class="empty-state-text">No significant matches found by NCBI</p></div>';
        }
        ...
        throw new Error('RID expired or invalid query parameters');
      }

      attempts++;
    }

    throw new Error('BLAST search timed out after 3 minutes (NCBI servers may be busy).');
